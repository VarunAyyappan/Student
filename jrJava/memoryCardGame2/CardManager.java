...
public class CardManager implements MouseListener {
	...
	public void applyGameLogic(){
		...
		if(!twoSelected || System.currentTimeMillis()<=timeTwoCardsSelected+2000)
			return;
		else if(firstSelected.equals(secondSelected)){
	...
	public void mousePressed(MouseEvent e) {
		...
		for(i=0; i<cards.length; i++){
			for(j=0; j<cards[i].length; j++){
				if(cards[i][j].isSelected(mx, my) && !cards[i][j].isMatched() && cards[i][j]!=firstSelected){
					cards[i][j].show();
					
					if(!oneSelected){ // meaning no card has been selected
						oneSelected = true;
					...
					else { // meaning one has been selected
						twoSelected = true;
						oneSelected = false;
